...
public class TextLayouter {
	...
	private void breakLine(BreakIterator iterator) {
		...
		for (int i = last.start; i < characters.length; i++) {
			Text tmp = MutableText.of(new LiteralTextContent(String.valueOf(characters[i]))).setStyle(last.span.style);
			width += textRenderer.getWidth(tmp);
			if (last.span.bold) {
				width++;
			}
	...
	private class SpanTail {
		...
		public SpanTail(Span span, int start, List<Word> cluster) {
			this.span = span;
			this.start = start;
			this.width = textRenderer.getWidth(span.styledSubstring(start)) + span.spacingLeft + span.spacingRight;
