...
public class GuiButtonBookBookmark extends GuiButtonBook {
	...
	public void renderButton(MatrixStack ms, int mouseX, int mouseY, float partialTicks) {
		...
		if (bookmark != null && entry != null) {
			...
			int px = x * 2 + (isHovered() || isFocused() ? 6 : 2);
			int py = y * 2 + 2;
	...
	private static Text[] getTooltip(Book book, Bookmark bookmark, boolean multiblock) {
		...
		if (bookmark == null || entry == null) {
			return new Text[] { MutableText.of(new TranslatableTextContent("patchouli.gui.lexicon.add_bookmark")) };
		}

		return new Text[] {
				entry.getName(),
				MutableText.of(new TranslatableTextContent(multiblock
						? "patchouli.gui.lexicon.multiblock_bookmark"
						: "patchouli.gui.lexicon.remove_bookmark")).setStyle(Style.EMPTY.withFormatting(Formatting.GRAY))
		};
	}
