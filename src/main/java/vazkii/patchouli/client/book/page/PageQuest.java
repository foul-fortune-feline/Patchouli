...
public class PageQuest extends PageWithText {
	...
	public void onDisplayed(GuiBookEntry parent, int left, int top) {
		...
		if (isManual) {
			ButtonWidget button = new ButtonWidget(GuiBook.PAGE_WIDTH / 2 - 50, GuiBook.PAGE_HEIGHT - 35, 100, 20, Text.empty(), this::questButtonClicked);
			addButton(button);
	...
	private void updateButtonText(ButtonWidget button) {
		...
		Text s = MutableText.of(new TranslatableTextContent(completed ? "patchouli.gui.lexicon.mark_incomplete" : "patchouli.gui.lexicon.mark_complete"));
		button.setMessage(s);
	}
